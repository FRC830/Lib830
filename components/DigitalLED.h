...
namespace Lib830 {
	...
	class DigitalLED {
	private:
		frc::DigitalOutput * red_channel;
		frc::DigitalOutput * green_channel;
		frc::DigitalOutput * blue_channel;
		frc::Timer timer;
		void Enable();
	...
	public:
		...
		struct Color { //RGB
			...
			Color operator+(const Color &c2) {
				Color a;
				a.red = (red + c2.red)/2;
				a.green = (green + c2.green)/2;
				a.blue = (blue + c2.blue)/2;
				return a;
			}
		...
		static Color hex2color(int hex);
		DigitalLED(frc::DigitalOutput * red, frc::DigitalOutput * green, frc::DigitalOutput * blue);
